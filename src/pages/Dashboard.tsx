    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10" ref={scrollRef}>
      <div className="gradient-hero-animated rounded-3xl p-6 sm:p-8 mb-8 text-white relative overflow-hidden animate-fade-in">
        <div className="absolute inset-0 opacity-10">
          ...
          <div className="absolute -bottom-10 -left-10 w-56 h-56 bg-sky-300/20 rounded-full blur-2xl" />
        </div>
        <div className="relative z-10 flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Zap className="text-sky-300" size={20} />
              <span className="text-sky-200 text-xs font-semibold uppercase tracking-widest">Dashboard</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold mb-1 tracking-tight">Welcome back, {user?.fullName?.split(' ')[0] || 'Student'}!</h1>
            <p className="text-sky-100 text-sm sm:text-base">Ready to continue your midwifery studies?</p>
          </div>
          <Sparkles className="text-sky-300 hidden sm:block animate-pulse-soft" size={32} />
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8">
        {[
          { icon: BookOpen, label: 'Subjects', value: totalSubjects, color: 'text-sky-600', bg: 'bg-sky-50', border: 'border-sky-100' },
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        <Link to="/practice" className="group card-modern p-5 border border-sky-100 hover:border-sky-200 scroll-reveal" style={{ animationDelay: '100ms' }}>
          <div className="w-12 h-12 gradient-primary rounded-2xl flex items-center justify-center mb-4 group-hover:shadow-glow-sky transition-shadow duration-300">
          ...
          <div className="mt-3 flex items-center gap-1 text-sky-600 text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {levelCounts.map((l, i) => (
            <Link to="/subjects" key={l.level} className="card-modern p-4 text-center group scroll-reveal" style={{ animationDelay: `${i * 80}ms` }}>
              <div className={`w-10 h-10 rounded-xl mx-auto mb-2 flex items-center justify-center text-sm font-bold text-white ${
                l.level === 100 ? 'bg-sky-500' : l.level === 200 ? 'bg-blue-500' : l.level === 300 ? 'bg-purple-500' : 'bg-amber-500'
              }`}>{l.level}</div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          <Link to="/subjects" className="text-sm text-sky-600 hover:text-sky-700 font-semibold flex items-center gap-1 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {subjects.filter(s => s.level === 200 || s.level === 300).slice(0, 6).map((subject, i) => {
            ...
            return (
              ...
              >
                <div className="w-11 h-11 bg-sky-50 rounded-xl flex items-center justify-center text-xl flex-shrink-0 group-hover:bg-sky-100 transition-colors">{subject.icon}</div>
                <div className="flex-1 min-w-0">
                  ...
                  {studied > 0 && (
                    <div className="mt-1 w-full h-1 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-sky-500 rounded-full" style={{ width: `${Math.round((studied / total) * 100)}%` }} />
                    </div>
                  )}
                </div>
                <span className="text-[11px] font-bold bg-sky-50 text-sky-600 px-2.5 py-0.5 rounded-full">{subject.level}L</span>
              </Link>
            )
          })}
        </div>
